...
模拟室使用一个不消耗的模拟目标和一组不消耗的模拟核心工作，不需要土壤或 FE。目标槽接受显式物品模拟配方、可自动识别的树苗、作物、矿物、神秘农业种子，或已绑定的数据模型；核心槽接受同一等级的可堆叠核心。
...
- `lazy:automatic_simulation_blacklist` 禁止目标的全部自动来源；`lazy:automatic_tree_blacklist`、`lazy:automatic_crop_blacklist`、`lazy:automatic_mineral_blacklist` 和 `lazy:automatic_mystical_blacklist` 只禁止对应来源。黑名单不影响显式配方，注入仍需已有基底。
