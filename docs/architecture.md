...
Compose 当前只提供模组入口、注册基础设施、数据生成入口和少量无业务含义的通用工具。物品、方块、维度、命令、菜单、能力、配方、贴图和语言资源都属于后续内容工作，在设计确认前不得进入运行源码。
...
`registerBlockWithItem` 用相同名称成对注册方块和物品，并返回两个延迟持有者。方块实体构建通过 `buildType` 收敛 Minecraft 1.21.1 Java API 的空安全边界。菜单、数据组件和创造模式标签页在出现真实用例后，通过各自模块的专用注册器扩展。
...
出现首个内容模块后，客户端 provider 与服务端 provider 分开装配。生成资源应保持可重复；缓存文件、Blockbench 工程文件和运行目录不得进入发布 JAR。
