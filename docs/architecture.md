...
Lazy 当前提供模组入口、注册基础设施、修复器、缓冲器、能量电池、能量源、物品复制器、虚空维度、配套传送器，以及少量无业务含义的通用工具。其他物品、方块和维度仍属于后续内容工作，在设计确认前不得进入运行源码。
...
Mojang Codec 继续用于数据驱动对象和跨版本结构化数据，NeoForge capability 继续作为物品、流体和能量互操作契约，`core.config` 对 NeoForge `ModConfigSpec` 提供最小 Kotlin 封装并管理服务器权威配置。这些原生契约不由 LDLib2 的 UI 或托管字段替代。
