---
navigation:
  parent: index.md
  title: Simulation Cores
  icon: lazy:simulation_core_t1
  position: 110
item_ids:
  - lazy:simulation_core_t1
  - lazy:simulation_core_t2
  - lazy:simulation_core_t3
  - lazy:simulation_core_t4
---

# Simulation Cores

<ItemImage id="lazy:simulation_core_t1" scale="1.4" />

Simulation Cores control the speed and output multiplier of the Simulation Chamber. The chamber accepts up to 64 cores, and cores are not consumed while it runs.

| Core | Speed | Output |
| --- | ---: | ---: |
| <ItemLink id="lazy:simulation_core_t1" /> | ×1 | ×1 |
| <ItemLink id="lazy:simulation_core_t2" /> | ×2 | ×4 |
| <ItemLink id="lazy:simulation_core_t3" /> | ×6 | ×12 |
| <ItemLink id="lazy:simulation_core_t4" /> | ×18 | ×36 |

These values are server configuration settings. Each core contributes its own speed and output multiplier, so adding cores scales both the simulation rate and virtual output rolls.
