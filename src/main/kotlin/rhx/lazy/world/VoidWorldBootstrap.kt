    ...
    fun bootstrapBiome(context: BootstrapContext<Biome>) {
        ...
        val biome =
            Biome
                ...
                .build()
        context.register(ModWorldKeys.voidBiome, biome)
    }

    fun bootstrapDimensionType(context: BootstrapContext<DimensionType>) {
        context.register(
            ModWorldKeys.voidDimensionType,
            DimensionType(
                OptionalLong.empty(),
                true,
                false,
        ...
        val generator =
            GridChunkGenerator(
                GridGeneratorSettings(
                    layerHeight = 128,
                    gridChunkSize = 3,
                    biome = biomes.getOrThrow(ModWorldKeys.voidBiome),
                    borderBlock = Blocks.STONE_BRICKS,
                    innerBlock = Blocks.SMOOTH_STONE,
                ),
            )
        context.register(
            ModWorldKeys.voidLevelStem,
            LevelStem(dimensionTypes.getOrThrow(ModWorldKeys.voidDimensionType), generator),
        )
    }
}
