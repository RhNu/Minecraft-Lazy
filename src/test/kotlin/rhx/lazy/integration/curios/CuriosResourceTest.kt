...
class CuriosResourceTest {
    ...
    @Test
    fun `configuration card slot is available to players and uses its strict validator`() {
        val slot = readJson("/data/lazy/curios/slots/configuration_card.json")
        assertEquals(1, slot["size"].asInt)
        assertEquals("lazy:slot/empty/empty_configuration_card_slot", slot["icon"].asString)
        assertEquals(listOf("lazy:configuration_card_slot"), slot["validators"].asJsonArray.map { it.asString })

        val entities = readJson("/data/lazy/curios/entities/configuration_card.json")
        assertEquals(listOf("minecraft:player"), entities["entities"].asJsonArray.map { it.asString })
        assertEquals(listOf("configuration_card"), entities["slots"].asJsonArray.map { it.asString })
    }

    @Test
    fun `configuration card slot texture is packaged and has an editable SVG source`() {
        assertNotNull(javaClass.getResourceAsStream("/assets/lazy/textures/slot/empty/empty_configuration_card_slot.png"))

        val projectRoot = Path.of(requireNotNull(System.getProperty("lazy.projectDir")))
        assertTrue(Files.isRegularFile(projectRoot.resolve("art/slot/empty/empty_configuration_card_slot.svg")))
    }
