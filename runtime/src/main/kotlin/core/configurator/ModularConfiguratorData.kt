...
internal class ModularConfiguratorData private constructor(
    ...
    public fun encodedMaterials(): List<ModularConfiguratorMaterialEntry> =
        materials.map { entry -> ModularConfiguratorMaterialEntry(entry.slot, entry.template, entry.count) }

    public fun encodedModulePayloads(): Map<ResourceLocation, CompoundTag> = modulePayloads.mapValues { (_, payload) -> payload.copy() }
